...
namespace OthelloDesktop.Models
{
    public class GameController(List<IPlayer> playerList)
    {
        ...
        public Action<Dictionary<Directions, Position>, Position, Piece> OnMoveMade;
        ...
        public void InitializeBoard()
        {
            ...
            OnMoveMade += (positionDictionary, targetPosition, color) =>
            {
                _board.Squares[targetPosition.Column, targetPosition.Row] = new Square(targetPosition, color, 1);
                foreach (var item in positionDictionary)
                {
                    Position iter = targetPosition;
                    while (!iter.Equals(item.Value))
                    {
                        iter += DirectionsDict[item.Key];
                        _board.Squares[iter.Column, iter.Row] = new Square(iter, color, 1);
                    }
                }
            };
        }
        ...
        public void MakeMove(Position piecePosition, Piece pieceColor)
        {
            ...
            if (IsValidMoves(piecePosition, pieceColor) && IsInsideBound(piecePosition))
            {
                ...
                OnMoveMade(filteredOrigins, piecePosition, pieceColor);
                /*
        ...
        public Piece GetSquarePiece(Position positionX)
        {
            return _board.Squares[positionX.Column, positionX.Row].SquarePiece;
        }

        public void SetSquarePiece(Position positionX, Piece pieceColor)
        {
            _board.Squares[positionX.Column, positionX.Row].SquarePiece = pieceColor;
        }
        /*
